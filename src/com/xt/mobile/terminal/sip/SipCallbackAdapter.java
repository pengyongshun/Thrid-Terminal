...
public abstract class SipCallbackAdapter implements XTSipClientInviteCallBack,
	...
	protected static final String TAG = SipCallbackAdapter.class.getSimpleName();
	...
	public void xt_sip_server_invite_offer_callback_t(long h, long msg, String sdp, int len) {
		...
		ToolLog.i("===SipCallbackAdapter===invite sdp:" + sdp);
		...
		if (SessionManager.getSessionBySessionId(sessionId) != null) {
			ToolLog.i("之前收到过点播，拒绝此次点播");
			SipManager.refuse(ch, SessionManager.REASON.Not_Acceptable_Here.code);
			return;
		}
		ToolLog.i("===SipCallbackAdapter===invite sessionId:" + sessionId + "; ch:" + ch);
		
		SessionManager.savePlaySessionMap(sessionId, sdp, ch);
	...
	public void xt_sip_server_invite_connected_confirmed_callback_t(long h, long msg) {
		...
		ToolLog.i("===SipCallbackAdapter===ack");
		
		String sessionId = SipNative.XtSipMsgGetCallID(msg);
		if (sessionId == null || sessionId.isEmpty()) {
			return;
		}
		ToolLog.i("===SipCallbackAdapter===ack sessionId:" + sessionId);

		if (SessionManager.getSessionBySessionId(sessionId) != null) {
			onPlaySuccess(sessionId);
		} else {
			ToolLog.i("===SipCallbackAdapter===ack unknow!");
		}
	}
	...
	public void xt_sip_server_invite_terminated_callback_t(long h, long msg, int reason) {
		...
		ToolLog.i("===SipCallbackAdapter===server_terminated reason：" + reason);

		String sessionId = SipNative.XtSipMsgGetCallID(msg);
		if (sessionId == null || sessionId.isEmpty()) {
			return;
		}
		ToolLog.i("===SipCallbackAdapter===server_terminated sessionId:" + sessionId);

		if (SessionManager.getSessionBySessionId(sessionId) != null) {
			onReceiveBye(sessionId);
		} else {
			ToolLog.i("===SipCallbackAdapter===server_terminated unknow!");
		}
	}
	...
	public void xt_sip_client_register_response_callback_t(long h, long msg, byte success) {
		...
		if (b) {
			...
			SipManager.registerHandler = SipNative.XtSipClientRegisterHandleClone(h);
		}
		onRegisterStatus(b);
	}
	...
	public void xt_sip_client_register_removed_callback_t(long h, long msg) {
		ToolLog.i("注销成功");
		SipNative.XtSipClientRegisterHandleDelete(SipManager.registerHandler);
	}
	...
	public abstract void onRegisterStatus(boolean success);

	public abstract void onReceivePlay(String sessionId);

	public abstract void onPlaySuccess(String sessionId);

	public abstract void onPlayFail(String sessionId);

	public abstract void onReceiveBye(String sessionId);
	...
	public void xt_sip_client_subscription_new_subscription_callback_t(long h, long notify) {
		SipManager.subscriptionHandle = SipNative.XtSipClientSubscriptionHandleClone(h);
	}
	...
	public void xt_sip_client_msg_prev_post_callback_t(long msg) {
		String id = SipManager.getContactsId(msg);
		ToolLog.i("有一个invite被发出了id:" + id);
